...
class DFExtender(pyspark.sql.dataframe.DataFrame):
    ...
    def compare_tables(self, df_ref: DataFrame):
        ...
        true_common_cols = compare_cols.get_intersection_groups(["all"])
        self._common_cols = compare_cols.get_intersection_groups(["all"], ["pk"])
        df1_cols = df_c1.get_columns_from_groups(["all"])
        df2_cols = df_c2.get_columns_from_groups(["all"])
